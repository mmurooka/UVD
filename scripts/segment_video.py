...
def write_boundary_yaml(
    ...
    fps: float,
    segmentation_config: sp.SegmentationConfig,
) -> str:
    import yaml

    metadata_path = build_metadata_path(output_path)
    boundary_secs = [0.0]
    internal_boundaries = np.array(milestone_indices[:-1], dtype=np.int64)
    boundary_secs.extend(round((int(idx) + 1) / fps, 6) for idx in internal_boundaries)
    metadata = {
        ...
        "segmentation_params": {
            ...
            **sp.algorithm_params_dict(segmentation_config),
        },
...
def main():
    args = parse_args()
    segmentation_config = sp.segmentation_config_from_args(args)
    video_file = os.path.expandvars(os.path.expanduser(args.video_file))
    output_path = build_output_path(video_file, args.render_suffix)
    ...
    cache_path = None if args.no_embedding_cache else sp.build_embedding_cache_path(
    ...
    indices = sp.segment_embeddings(embeddings=rep, config=segmentation_config)
    print(
    ...
    save_video_ffmpeg(rendered, output_path, fps=fps)
